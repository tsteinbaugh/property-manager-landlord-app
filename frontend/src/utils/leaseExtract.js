export function extractLeaseFields(rawText) {
  ...
  const fields = {
    ...
    lastMonthPrepaid: false,
    graceDays: 7,
    lateFeePolicy: { type: "flat", value: 0 },
    otherRecurring: [],
  };
  const matches = {};
  ...
  const toISO = (m, d, y) => {
    const yy = Number(y.length === 2 ? (Number(y) > 70 ? `19${y}` : `20${y}`) : y);
    const date = new Date(yy, Number(m) - 1, Number(d));
  ...
  const tryDate = (s) => {
    if (!s) return null;
    let m = s.match(dateMDY);
    if (m) return toISO(m[1], m[2], m[3]);
    m = s.match(dateLong);
    if (m) return toISO(monthMap[m[1]], m[2], m[3]);
    return null;
  };
  ...
  if (startISO) {
    fields.startDateISO = startISO;
    matches.startDate = { value: startISO, conf: 0.6 };
  }
  ...
  if (!fields.months && startISO && endISO) {
    ...
    if (months > 0 && months < 60) {
      ...
      matches.months = { value: months, conf: 0.55 };
    }
  }

  const mDue = text.match(
    /\b(?:rent\s*is\s*due|due\s*date|on\s*the)\s*(\d{1,2})(?:st|nd|rd|th)?\s*(?:of\s*each\s*month|monthly)?/i,
  );
  if (mDue) {
    fields.dueDay = Number(mDue[1]);
    matches.dueDay = { value: fields.dueDay, conf: 0.8 };
  }

  const mRent = text.match(
    /\b(?:monthly\s*rent|rent\s*amount|rent)\s*(?:is|:)?\s*\$?\s*([0-9,]+(?:\.[0-9]{2})?)/i,
  );
  if (mRent) {
    fields.monthlyRent = currency(mRent[1]);
    matches.monthlyRent = { value: fields.monthlyRent, conf: 0.85 };
  }

  const mPetRent = text.match(/\bpet\s*rent\b[^\n$]*\$\s*([0-9,]+(?:\.[0-9]{2})?)/i);
  if (mPetRent) {
    fields.petRentEnabled = true;
    fields.petRentAmount = currency(mPetRent[1]);
    matches.petRent = { value: fields.petRentAmount, conf: 0.75 };
  }

  const mSec = text.match(
    /\b(?:security\s*deposit|deposit)\s*(?:is|:)?\s*\$?\s*([0-9,]+(?:\.[0-9]{2})?)/i,
  );
  if (mSec) {
    fields.securityDeposit = currency(mSec[1]);
    matches.securityDeposit = { value: fields.securityDeposit, conf: 0.85 };
  }

  if (/\bfirst\s+month(?:'s)?\s+(?:rent\s+)?prepaid\b/i.test(text)) {
    fields.firstMonthPrepaid = true;
  }
  if (/\blast\s+month(?:'s)?\s+(?:rent\s+)?prepaid\b/i.test(text)) {
    fields.lastMonthPrepaid = true;
  }

  const mGrace = text.match(/\bgrace\s*period\b[^\d]*(\d{1,2})\s*day/i);
  if (mGrace) {
    fields.graceDays = Number(mGrace[1]);
  }

  const mLateFlat = text.match(/\blate\s*fee\b[^%\n]*\$\s*([0-9,]+(?:\.[0-9]{2})?)/i);
  const mLatePct = text.match(/\blate\s*fee\b[^\n]*?(\d{1,2})\s*%/i);
  if (mLateFlat) {
    fields.lateFeePolicy = { type: "flat", value: currency(mLateFlat[1]) };
  } else if (mLatePct) {
    fields.lateFeePolicy = { type: "percent", value: Number(mLatePct[1]) };
  }
