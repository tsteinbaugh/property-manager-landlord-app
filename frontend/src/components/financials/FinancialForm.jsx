...
export default function FinancialForm({
  ...
  const [monthsStr, setMonthsStr] = useState(
    initialValues.months != null ? String(initialValues.months) : "12",
  );
  const [dueDayStr, setDueDayStr] = useState(
    initialValues.dueDay != null ? String(initialValues.dueDay) : "1",
  );
  const [monthlyRentStr, setMonthlyRentStr] = useState(
    initialValues.monthlyRent != null ? String(initialValues.monthlyRent) : "0.00",
  );

  const [petsEnabled, setPetsEnabled] = useState(!!initialValues.petRentEnabled);
  const [petRentAmountStr, setPetRentAmountStr] = useState(
    initialValues.petRentAmount != null ? String(initialValues.petRentAmount) : "0.00",
  );

  const [securityDepositStr, setSecurityDepositStr] = useState(
    initialValues.securityDeposit != null
      ? String(initialValues.securityDeposit)
      : "0.00",
  );
  const [secDepEqualsRent, setSecDepEqualsRent] = useState(
    !!initialValues.secDepEqualsRent,
  );
  const [petDepositStr, setPetDepositStr] = useState(
    initialValues.petDeposit != null ? String(initialValues.petDeposit) : "0.00",
  );

  const [securityDepositPayments, setSecurityDepositPayments] = useState(
    Array.isArray(initialValues.securityDepositPayments)
      ? initialValues.securityDepositPayments.map((p) => ({
          ...p,
          amount: Number(p.amount) || 0,
        }))
      : initialValues.securityDepositPayment
        ? [
            {
              ...initialValues.securityDepositPayment,
              amount: Number(initialValues.securityDepositPayment.amount) || 0,
            },
          ]
        : [],
  );
  const [petDepositPayments, setPetDepositPayments] = useState(
    Array.isArray(initialValues.petDepositPayments)
      ? initialValues.petDepositPayments.map((p) => ({
          ...p,
          amount: Number(p.amount) || 0,
        }))
      : initialValues.petDepositPayment
        ? [
            {
              ...initialValues.petDepositPayment,
              amount: Number(initialValues.petDepositPayment.amount) || 0,
            },
          ]
        : [],
  );

  const [firstMonthPrepaid, setFirstMonthPrepaid] = useState(
    !!initialValues.firstMonthPrepaid,
  );
  const [lastMonthPrepaid, setLastMonthPrepaid] = useState(
    !!initialValues.lastMonthPrepaid,
  );
  const [firstMonthPayments, setFirstMonthPayments] = useState(
    Array.isArray(initialValues.firstMonthPayments)
      ? initialValues.firstMonthPayments.map((p) => ({
          ...p,
          amount: Number(p.amount) || 0,
        }))
      : initialValues.firstMonthPayment
        ? [
            {
              ...initialValues.firstMonthPayment,
              amount: Number(initialValues.firstMonthPayment.amount) || 0,
            },
          ]
        : [],
  );
  const [lastMonthPayments, setLastMonthPayments] = useState(
    Array.isArray(initialValues.lastMonthPayments)
      ? initialValues.lastMonthPayments.map((p) => ({
          ...p,
          amount: Number(p.amount) || 0,
        }))
      : initialValues.lastMonthPayment
        ? [
            {
              ...initialValues.lastMonthPayment,
              amount: Number(initialValues.lastMonthPayment.amount) || 0,
            },
          ]
        : [],
  );

  const [graceDaysStr, setGraceDaysStr] = useState(
    initialValues.graceDays != null ? String(initialValues.graceDays) : "7",
  );
  const [lateFeeType, setLateFeeType] = useState(
    initialValues.lateFeePolicy?.type || "flat",
  );
  const [lateFeeValueStr, setLateFeeValueStr] = useState(
    initialValues.lateFeePolicy?.value != null
      ? String(initialValues.lateFeePolicy.value)
      : "50.00",
  );
  ...
  const [petDepDueSigning, setPetDepDueSigning] = useState(
    !!initialValues.petDepDueSigning,
  );
  ...
  const [signingPayment, setSigningPayment] = useState(
    initialValues.signingPayment || null,
  );
  ...
  useEffect(() => {
    if (secDepEqualsRent) setSecurityDepositStr(monthlyRentStr);
  }, [monthlyRentStr, secDepEqualsRent]);

  /* due-at-signing breakdown */
  const signingBreakdown = useMemo(() => {
    const lines = [];
    if (secDueSigning && securityDeposit > 0)
      lines.push({ key: "sec", label: "Security deposit", amount: securityDeposit });
    if (petsEnabled && petDepDueSigning && petDeposit > 0)
      lines.push({ key: "pet", label: "Pet deposit", amount: petDeposit });
    if (firstDueSigning && firstMonthDue > 0)
      lines.push({ key: "first", label: "First month", amount: firstMonthDue });
    if (lastDueSigning && lastMonthDue > 0)
      lines.push({ key: "last", label: "Last month", amount: lastMonthDue });
    return lines;
  }, [
    secDueSigning,
    petsEnabled,
  ...
  const signingTotal = useMemo(
    () => signingBreakdown.reduce((s, l) => s + (Number(l.amount) || 0), 0),
    [signingBreakdown],
  );
  ...
  function applySigningPayment(p) {
    ...
    const take = (need) => {
      ...
      const amt = Math.min(remaining, Number(need) || 0);
    ...
    if (secDueSigning && securityDeposit > 0) {
      const a = take(securityDeposit);
      if (a > 0) nextSec.push({ amount: a, ...base });
    }
    if (petsEnabled && petDepDueSigning && petDeposit > 0) {
      const a = take(petDeposit);
      if (a > 0) nextPet.push({ amount: a, ...base });
    }
    if (firstDueSigning && firstMonthDue > 0) {
      const a = take(firstMonthDue);
      if (a > 0) nextFirst.push({ amount: a, ...base });
    }
    if (lastDueSigning && lastMonthDue > 0) {
      const a = take(lastMonthDue);
      if (a > 0) nextLast.push({ amount: a, ...base });
    }
  ...
  const errors = useMemo(() => {
    ...
    if (cfg.petRentEnabled && Number(cfg.petDeposit) < 0)
      e.petDeposit = "Pet deposit cannot be negative.";
  ...
  function openPaymentsFor(key) {
    const map = {
      sec: securityDepositPayments,
      pet: petDepositPayments,
      first: firstMonthPayments,
      last: lastMonthPayments,
    };
    const arr = map[key] || [];
    ...
    >
      <div className={styles.card}>
        ...
        <div className={styles.grid}>
          ...
          <div className={styles.sectionBox}>
            <div className={styles.sectionHeader}>
              <h4 className={styles.sectionTitle}>Security Deposit</h4>
            </div>
            <div className={styles.sectionBody}>
              <div className={styles.fieldBox}>
                <div className={styles.fieldBoxHeader}>
                  <div className={styles.fieldBoxTitle}>
                    Equals one month’s base rent?
                  </div>
                  <label
                    className={styles.switch}
                    title="When ON, Security Deposit mirrors Monthly Base Rent and becomes read-only."
                  >
                    <input
                      type="checkbox"
                      ...
                      onChange={(e) => {
                        ...
                        setSecDepEqualsRent(checked);
                        if (checked) setSecurityDepositStr(monthlyRentStr);
                      }}
                    />
                ...
                <div className={styles.sectionHelp}>
                  When enabled, the Security Deposit field mirrors Monthly Base Rent and
                  can’t be edited.
                </div>
              </div>

              <div
                className={secDepEqualsRent ? styles.readonly : ""}
                title={secDepEqualsRent ? "Turn off the toggle above to edit" : ""}
              >
                <FloatingField
                  type="text"
                  ...
                  inputProps={{
                    inputMode: "decimal",
                    readOnly: secDepEqualsRent,
                    disabled: secDepEqualsRent,
                  }}
                />
              </div>
                ...
                <>
                  ...
                  <div className={styles.leftActions}>
                    <button
                      ...
                      disabled={
                        securityDeposit <= 0 || (!!signingPayment && secDueSigning)
                      }
                      title={
                        securityDeposit <= 0
                    ...
                    {securityDepositPayments.length > 0 && (
                      <div className={styles.subtle}>
                        Received: ${sumPayments(securityDepositPayments).toFixed(2)}
                      </div>
                    )}
                  </div>
                </>
          ...
          <div className={styles.sectionBox}>
            <div className={styles.sectionHeader}>
              <h4 className={styles.sectionTitle}>Pets</h4>
            </div>
            <div className={styles.sectionBody}>
              <div className={styles.fieldBox}>
                <div className={styles.fieldBoxHeader}>
                  ...
                  {(toNum(petDepositStr) > 0 || petDepositPayments.length > 0) && (
                    <>
                      ...
                      <div className={styles.leftActions}>
                        ...
                        {petDepositPayments.length > 0 && (
                          <div className={styles.subtle}>
                            Received: ${sumPayments(petDepositPayments).toFixed(2)}
                          </div>
                        )}
                      </div>
                    </>
          ...
          <div className={styles.sectionBox}>
            <div className={styles.sectionHeader}>
              <h4 className={styles.sectionTitle}>First / Last Month</h4>
            </div>
            <div className={styles.sectionBody}>
                    ...
                    <label className={styles.switch}>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setFirstMonthPrepaid(c);
                          if (!c) setFirstMonthPayments([]);
                        }}
                      />
                      <span className={styles.slider} />
                    </label>
                    ...
                    <>
                      ...
                      <div className={styles.leftActions}>
                        <button
                          ...
                          title={
                            signingPayment && firstDueSigning
                        ...
                        {firstMonthPayments.length > 0 && (
                          <div className={styles.subtle}>
                            Received: ${sumPayments(firstMonthPayments).toFixed(2)}
                          </div>
                        )}
                      </div>
                    </>
                  ...
                  <div className={styles.colHeader}>
                    ...
                    <label className={styles.switch}>
                      <input
                        ...
                        onChange={(e) => {
                          const c = e.target.checked;
                          setLastMonthPrepaid(c);
                          if (!c) setLastMonthPayments([]);
                        }}
                      />
                      <span className={styles.slider} />
                    </label>
                  ...
                  {lastMonthPrepaid && (
                    <>
                      ...
                      <div className={styles.leftActions}>
                        ...
                        {lastMonthPayments.length > 0 && (
                          <div className={styles.subtle}>
                            Received: ${sumPayments(lastMonthPayments).toFixed(2)}
                          </div>
                        )}
                      </div>
                    </>
          ...
          <div className={styles.sectionBox}>
            <div className={styles.sectionHeader}>
              <h4 className={styles.sectionTitle}>Late Fees</h4>
            </div>
            <div className={styles.sectionBody}>
              <FloatingField
                type="number"
            ...
            <div className={styles.sectionBody}>
              <div className={styles.stackToggles}>
                <label className={styles.stackRow}>
                  ...
                  <label className={styles.switch}>
                    <input
                      type="checkbox"
                      checked={secDueSigning}
                      onChange={(e) => setSecDueSigning(e.target.checked)}
                    />
                    <span className={styles.slider} />
                  </label>
                </label>
                ...
                {petsEnabled && (
                  <label className={styles.stackRow}>
                    ...
                    <label className={styles.switch}>
                      <input
                        type="checkbox"
                        checked={petDepDueSigning}
                        onChange={(e) => setPetDepDueSigning(e.target.checked)}
                      />
                      <span className={styles.slider} />
                    </label>
                  </label>
                ...
                {signingBreakdown.length === 0 ? (
                  ...
                  signingBreakdown.map((l) => (
                    <div key={l.key} className={styles.breakdownRow}>
                      <span>{l.label}</span>
                      <strong>${(Number(l.amount) || 0).toFixed(2)}</strong>
                    </div>
                  ))
                )}
              </div>
              ...
              {signingPayment && (
                <div className={styles.help} style={{ marginTop: 6 }}>
                  Signing payment: ${Number(signingPayment.amount).toFixed(2)} on{" "}
                  {signingPayment.dateISO}
                  {signingPayment.method ? ` · ${signingPayment.method}` : ""}
                  {signingPayment.note ? ` — ${signingPayment.note}` : ""}
                </div>
              )}
            </div>
          </div>
        ...
        <PaymentModal
          ...
          title={
            openQuickAdd === "sec"
              ...
              : openQuickAdd === "pet"
                ? "Add Pet Deposit Payment"
                : openQuickAdd === "first"
                  ? "Add First Month Payment"
                  : openQuickAdd === "last"
                    ? "Add Last Month Payment"
                    : openQuickAdd === "signing"
                      ? "Record Signing Payment"
                      : "Add Payment"
          }
          expectedAmount={
            openQuickAdd === "sec"
      ...
      {openManageKey && (
        <ManagePaymentsModal
          ...
          title={
            openManageKey === "sec"
              ? "Manage Payments — Security Deposit"
              : openManageKey === "pet"
                ? "Manage Payments — Pet Deposit"
                : openManageKey === "first"
                  ? "Manage Payments — First Month"
                  : openManageKey === "last"
                    ? "Manage Payments — Last Month"
                    : "Manage Payments"
          }
          payments={
            openManageKey === "sec"
