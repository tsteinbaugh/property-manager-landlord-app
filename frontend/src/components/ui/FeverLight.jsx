...
export default function FeverLight({ state = "unknown", color, size = 14, title }) {
  const mapStateToColor = (s) => {
    switch (s) {
      ...
      case "late_paid": return "yellow"; // keep dashboard workflow default
  ...
  const label = title || (state === "on_time" ? "On time" : state === "late_paid" ? "Late / partial" : state === "unpaid" ? "Unpaid" : "Unknown");
