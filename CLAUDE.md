...
**Standing backlog, no active priority order right now — ask Taylor what's next:** Move-in/Move-out Inspections, Legal Tracker (Colorado-only first, per "Nationwide jurisdictional coverage plan" below), a maintenance-supplies inventory idea (memory `project_maintenance_supplies_inventory_idea`), expanding lease clause state coverage (tabled, see above), **property archiving** (surfaced 2026-08-19 while seeding Rent Tracker demo data — see Known issues below for the underlying bug this was found alongside), and a **property-level for-cause-eviction-exemption attribute** (surfaced 2026-08-20/21 during the CO clause-library verification pass — `lease-clause-decision-log-CO.md` §8a, Part 13). The Lease Builder now ships two month-to-month termination-notice clause variants (`month-to-month-notice-co-exempt` / `month-to-month-notice-co-covered`), but nothing yet drives which one applies — the landlord has to pick the right one by hand on every CO lease. The proposed fix (not yet built) is a `Property`-level field asking "I live in this property, or a property directly next door to it, as my own home" / "This is a short-term rental (Airbnb/VRBO-style)" / "Neither," set once at property setup like the rest of the property-attributes work, that the Lease Builder then uses to auto-select the right variant when attaching. Full option text and the underlying statute (C.R.S. § 38-12-1302) are in the decision log entry above.
