...
All six v1 MVP scope items (Entities, Properties, Tenants + Leases, Finances, Maintenance, User auth) are built end-to-end, tagged `v1.0.0`. Since then: global fuzzy/phonetic search, and a real architectural fix — Occupant/Pet/Vehicle now follow the Tenant instead of the Lease, and every top-level nav page (Tenants/Leases/Finances/Maintenance) got full add/edit parity with the property page, not just read-only links. Backend is at 219 tests. **Deploying is deliberately on hold** — Taylor's choice, no forcing function while still using the app locally; flip that switch whenever mobile access or always-on actually matters, not before. **Next up: Property Specs**, then Lease Builder — see the v2 roadmap priority order agreed 2026-08-16 (full reasoning in memory: Property Specs → Lease Builder → small computed-status automation → Inspections → the real rent/late-fee automation engine (naturally follows deploy) → Legal Tracker last, deliberately, given its legal-accuracy risk).
...
*Last updated: 2026-08-16 — Global search, Occupant/Pet/Vehicle re-scoped to Tenant (with Tenant-page CRUD fixing the pre-lease chicken-and-egg problem), and full add/edit parity added to all top-level nav pages (Tenants/Leases/Finances/Maintenance), all with Claude Code — no open blockers, next up is deployment (Vercel + Railway), then let real usage decide the v2 priority*
