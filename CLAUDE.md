...
- [x] [Aug 2026] — `tenants.routes.js` — full CRUD scoped to the authenticated user; creating a tenant validates the named property belongs to you; `applicationStatus` is updatable (validated against the enum) so Taylor can approve/reject applicants. Mounted at `/api/tenants`. 12 tests passing.
