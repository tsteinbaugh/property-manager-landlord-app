...
All six v1 MVP scope items (Entities, Properties, Tenants + Leases, Finances, Maintenance, User auth) are built end-to-end, tagged `v1.0.0`. Since then: global fuzzy/phonetic search, a real architectural fix (Occupant/Pet/Vehicle follow the Tenant instead of the Lease), full add/edit parity on every top-level nav page, **Property Specs (v2)** — 7 categories, By Room / By Category dual view, expense linking, maintenance linking, retire/replace lifecycle — and now **Lease Builder (v2) is built**: a personal clause library (verbatim text, section number, category tag, an `isEarlyTermination` flag), a starter-template set, attaching clauses to a lease as an immutable snapshot, a missing-early-termination-clause warning, and generating a real PDF from the lease's key terms + clauses that flows into the existing document-storage/download UI. Also fixed app-wide: "Back to X" links on detail pages now use real browser history (`BackLink` component) instead of a hardcoded destination, since several pages (Tenant, Lease, Property) are reachable from more than one place. Backend is at 315 tests. **Deploying is deliberately on hold** — Taylor's choice, no forcing function while still using the app locally.
...
- [Aug 2026] — **Lease Builder (v2) built** — clause library, template starter set, attaching clauses to a lease, a missing-early-termination-clause warning, and generating a real lease PDF. New `Clause` model (`userId`-scoped personal library — verbatim `bodyText`, `sectionNumber`, free-form `category` tag, a dedicated `isEarlyTermination` boolean) and `LeaseClause` (clauses attached to one lease, **snapshotted** at attach time — own copies of every field, plus a nullable `sourceClauseId` for traceability only). Snapshotting, not a live reference, is the load-bearing decision here: once a lease is generated — potentially signed, potentially the document Taylor relies on in court — editing the master library clause later must never silently rewrite what a past lease says, same instinct as Property Specs' retire/replace lifecycle. `backend/src/lib/clauseTemplates.js` is a static, hardcoded starter set (not DB rows — no per-user/global template modeling needed for a fixed reference list), served read-only at `GET /api/clause-templates`; "importing" one just prefills the add-clause form so the landlord edits before saving their own copy. `POST /api/leases/:id/clauses` accepts either `{ clauseId }` (copy from library) or a custom one-off `{ title, bodyText, ... }` that never touches the library; `PUT`/`DELETE` edit or remove a lease's own snapshot. `GET` on a lease now includes a compute-on-read `missingEarlyTerminationClause` flag (true only when the lease has ≥1 clause and none is flagged early-termination — a pure v1 upload-only lease with zero clauses stays silent, since there's nothing to inspect). `POST /api/leases/:id/generate-document` assembles the lease's key terms (dates, rent, deposit, late fee, pet policy) plus ordered clause text into a real PDF via `pdfkit` (`backend/src/lib/generateLeasePdf.js`) and uploads it through a new `r2.putObject` helper (direct backend-side upload, as opposed to the existing presigned-PUT-from-browser flow) — reusing the exact same document-key-replace/download machinery a v1 uploaded PDF already uses, so no new document UI was needed. Frontend: `ClauseLibraryPage` (`/clauses`, linked from the Leases hub) for managing the library and browsing/importing templates; `LeaseDetailPage` gained a `LeaseBuilderSection` (attach from library via a searchable picker, or add a custom clause; edit/remove; the warning banner; "Generate Lease PDF"). Verified end-to-end against the real dev DB and live R2 bucket (not just the mocked test suite) — a real PDF was generated, uploaded, downloaded via presigned URL, and confirmed valid. Deliberately deferred: the violation builder and clause→legal-action linking, both of which depend on Legal Tracker's own data shape (last in the v2 roadmap, precisely because of its legal-accuracy risk) — see memory `project_v2_roadmap_priority`. 24 new backend tests, 315 total.
...
- [Aug 2026] — `LeaseClause` snapshots a `Clause`'s fields at attach time rather than referencing it live. Reason: once a lease has been generated (and potentially signed, potentially the document Taylor relies on in court), editing the master library clause afterward must never silently rewrite what that past lease says. `sourceClauseId` is kept only for traceability, `onDelete: SetNull` — deleting a library clause never touches leases it's already attached to. Same instinct as Property Specs' retire/replace lifecycle and the immutable `MaintenanceStatusChange`/`MaintenanceScheduleCompletion` audit rows. Do not switch `LeaseClause` to a live join/reference — that would be a legal-accuracy regression, not a simplification.
...
*Last updated: 2026-08-17 — Lease Builder (v2) built: clause library, template starter set, snapshot-on-attach lease clauses, missing-early-termination warning, real PDF generation. App-wide "Back to X" navigation bug also fixed. No open blockers, next up is small computed-status automation per the v2 roadmap.*
