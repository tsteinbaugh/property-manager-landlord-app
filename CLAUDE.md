...
1. **Automate Finances** — the "real automation engine" item from the original v2 roadmap (recurring rent charges so "expected vs. collected" becomes real instead of eyeballed, auto-applied late fees), now moved up rather than waiting on deploy. Revisit with Taylor whether it still needs deploy as a hard prerequisite (the original reasoning was that unattended background actions like flipping a status or applying a fee at midnight need something running 24/7) or whether a smaller first slice can work without that.
...
**Standing backlog, lower priority than the above:** Move-in/Move-out Inspections, Legal Tracker (Colorado-only first, per "Nationwide jurisdictional coverage plan" below), a maintenance-supplies inventory idea (memory `project_maintenance_supplies_inventory_idea`), and expanding lease clause state coverage (tabled, see above). Full original v2 roadmap reasoning still in memory `project_v2_roadmap_priority` for reference, though the ordering above supersedes it for now.
