...
All six v1 MVP scope items (Entities, Properties, Tenants + Leases, Finances, Maintenance, User auth) are built end-to-end, tagged `v1.0.0`. Since then: global fuzzy/phonetic search, a real architectural fix (Occupant/Pet/Vehicle follow the Tenant instead of the Lease), full add/edit parity on every top-level nav page, and **Property Specs (v2) is now fully built, including a round of refinements** — 7 categories (Paint, Flooring, Countertops/"Appliances & Systems"/etc.), By Room / By Category dual view at `/properties/:id/specs`, spec items link to the real Expense that paid for them (Flooring/Countertop/Backsplash) and to any Maintenance record (all 7 categories, with read-only history shown on the item), plus a retire/replace lifecycle so replacing something never silently loses its prior history. Backend is at 291 tests. **Deploying is deliberately on hold** — Taylor's choice, no forcing function while still using the app locally.

**Next up: Lease Builder** — see the v2 roadmap priority order (memory `project_v2_roadmap_priority`): Property Specs (done) → Lease Builder → small computed-status automation → Inspections → the real rent/late-fee automation engine (follows deploy) → Legal Tracker last, deliberately, given its legal-accuracy risk. Also on record but explicitly low priority: a maintenance-supplies inventory idea (memory `project_maintenance_supplies_inventory_idea`).
...
*Last updated: 2026-08-16 — Property Specs (v2) built and then refined (expense linking, house systems, maintenance linking across all 7 categories, retire/replace lifecycle) on top of global search and the Occupant/Pet/Vehicle/CRUD-parity work from earlier the same day, all with Claude Code — no open blockers, next up is Lease Builder per the v2 roadmap*
