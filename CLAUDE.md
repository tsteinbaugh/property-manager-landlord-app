...
All six v1 MVP scope items (Entities, Properties, Tenants + Leases, Finances, Maintenance, User auth) are built end-to-end, tagged `v1.0.0`. Since then: global fuzzy/phonetic search, a real architectural fix (Occupant/Pet/Vehicle follow the Tenant instead of the Lease), full add/edit parity on every top-level nav page, and **Property Specs (v2) is now built** — 7 categories (Paint, Flooring, Countertops, Fixtures, Appliances, Backsplash, Exterior/Grounds), By Room / By Category dual view, at `/properties/:id/specs`. Backend is at 283 tests. **Deploying is deliberately on hold** — Taylor's choice, no forcing function while still using the app locally.

**In progress / agreed but not yet built** (from a 2026-08-16 walkthrough of Property Specs): (1) drop `ExteriorFeature`'s `serviceContractor`/`serviceContractType`/`serviceCost` fields — they duplicate what `MaintenanceSchedule` + `Expense` already cover, a bug introduced during the breadth-first build, not a real design decision; (2) add `ExpenseCategory.IMPROVEMENT` plus an `expenseId` link on `FlooringSpec`/`CountertopSpec`/`BacksplashSpec` (replacing their standalone `cost` field entirely — one Expense can be linked from several spec items, e.g. one remodel invoice covering new flooring + countertop + backsplash; no separate cost field to keep in sync); (3) broaden the "Appliances" label to "Appliances & Systems" in the UI only (house systems like furnace/water heater/HVAC fit the existing `Appliance` fields as-is — no new Prisma model, same reasoning as collapsing Fixture's 5 sub-types into one model with a type enum); (4) add optional `applianceId`/`fixtureId` links on `MaintenanceRequest`/`MaintenanceSchedule` (Appliances/Systems and Fixtures only — not the other 5 categories, which aren't really "serviced") so maintenance history can show read-only on the spec item; (5) a retire/replace lifecycle (`active`/`retiredAt`) scoped to Appliances/Systems and Fixtures — "Replace" creates a new spec item and marks the old one retired instead of editing in place, so maintenance history stays correctly attached to whichever physical unit it actually happened to (same "don't silently lose history" instinct as `MaintenanceStatusChange`/`MaintenanceScheduleCompletion`). **Next: build these five**, then continue down the v2 roadmap (Lease Builder next — see memory `project_v2_roadmap_priority`).
...
*Last updated: 2026-08-16 — Property Specs (v2) built (7 categories, By Room / By Category dual view) on top of global search and the Occupant/Pet/Vehicle/CRUD-parity work from earlier the same day, all with Claude Code — a walkthrough of Property Specs surfaced 5 agreed refinements not yet built (see Current focus), do those next before moving further down the v2 roadmap*
