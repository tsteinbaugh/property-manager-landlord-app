...
All six v1 MVP scope items (Entities, Properties, Tenants + Leases, Finances, Maintenance, User auth) are now built end-to-end — backend (201 tests) and a full frontend, both tested and clicked through. **v1 MVP is considered wrapped.** Next: not yet decided — candidates are deploying (Vercel + Railway, tech stack table below is stale on this), starting v2 features (Property Specs is the most natural next module), or continuing to refine v1 from real personal usage. Ask Taylor before picking a direction.
...
**v1 MVP complete as of this entry** — all six scope items from CLAUDE.md's MVP list are built and working end-to-end. 201 backend tests passing.
...
*Last updated: 2026-08-16 — Full v1 MVP frontend built out (Entities, Properties, Tenants, Leases, Finances, Maintenance, plus Occupants/Pets/Vehicles and a restructured side nav) with Claude Code — v1 MVP is considered wrapped end-to-end, no open blockers, next direction not yet decided (deploy, v2 features, or refine from real usage)*
