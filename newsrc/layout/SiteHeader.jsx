...
export default function SiteHeader() {
  ...
  const { query, setQuery, open, setOpen, loading, results } = useGlobalSearch({
    role: effectiveRole,   // search obeys impersonation
  ...
  return (
    ...
    >
      <Breadcrumbs items={[{ label: "Home", to: "/" }, { label: "Dashboard", to: "/dashboard" }]} />
      <div style={{ flex: "0 1 520px" }}>
        <SearchInput
          ...
          results={results.map(r => ({
            ...
            sublabel: [
              r.item.propertyName, r.item.email, r.item.phone, r.item.address, r.item.city, r.item.type, r.item.breed, r.item.relation,
