...
function parseEmergencyContactPost(body) {
  ...
  if (address1 === "__INVALID__") return { error: "address1 must be a string" };

  const city = optionalTrimToNull(src.city);
  if (city === "__INVALID__") return { error: "city must be a string" };
...
function parseEmergencyContactPatch(body, { existing } = {}) {
  ...
  if (src.address1 !== undefined) {
    ...
    if (v === "__INVALID__") return { error: "address1 must be a string" };
    data.address1 = v;
  }

  if (src.city !== undefined) {
    const v = optionalTrimToNull(src.city);
    if (v === "__INVALID__") return { error: "city must be a string" };
    data.city = v;
  }
